...
class SalonController extends Controller {
  ...
  public function show($params){
    ...
    // Fetch stylists
    $sty = $pdo->prepare('SELECT id, full_name AS fullName, avatar, bio, rating_avg AS ratingAvg, rating_count AS ratingCount, active FROM stylists WHERE salon_id = ? AND active = 1 ORDER BY full_name');
    $sty->execute([$id]);
    $stylists = $sty->fetchAll(\PDO::FETCH_ASSOC);

    // Fetch recent reviews (if reviews table exists)
    ...
    foreach($reviews as &$rr) { $rr['rating'] = isset($rr['rating']) ? (int)$rr['rating'] : 0; }

    return $this->json(['salon'=>$s, 'services'=>$services, 'stylists'=>$stylists, 'reviews'=>$reviews]);
  }
}
