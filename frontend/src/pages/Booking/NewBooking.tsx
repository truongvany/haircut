...
import type { PaymentMethod } from '../Payment/PaymentForm';
...
export default function NewBookingPage() {
    ...
    const [pendingBookingPayload, setPendingBookingPayload] = useState<CreateBookingPayload | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Save payload and show payment form (don't create booking yet)
        const { subtotal: sub } = computeTotals();
        const discount = appliedVoucher ? (appliedVoucher.discount_amt ?? Math.floor((appliedVoucher.discount_pct ?? 0)/100 * sub)) : 0;
    ...
    const handlePaymentSubmit = async (method: PaymentMethod) => {
        if (!pendingBookingPayload) {
            setError('Lỗi: Dữ liệu lịch không tìm thấy');
            return;
        }
        ...
        try {
            ...
            const bookingResponse = await createBooking(pendingBookingPayload);
            const createdBookingId = bookingResponse.booking_id;
            setBookingId(createdBookingId);

            // Step 2: Create payment record
            const paymentResponse = await createPayment(createdBookingId, method);
            ...
            setError(err?.response?.data?.error || 'Không thể xử lý thanh toán');
            setShowPaymentForm(false);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="new-booking-container">
            ...
            {/* Payment Form Modal */}
            {showPaymentForm && pendingBookingPayload && (
                <PaymentForm
                    amount={paymentTotal}
                    bookingId={0}
            ...
            {showPaymentSuccess && bookingId && selectedPaymentMethod && (
                <PaymentSuccess
                    bookingId={bookingId}
                    amount={paymentTotal}
